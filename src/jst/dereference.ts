import { merge, iterate, contains } from "./object";
// import uri from "valid-url";
import isUri from "@lvchengbin/is/src/url";
 ...
 */
const dereference = (schema, resolve?) => {
      ...
      iterate(node, (key, value) => {
        ...
        // it into the base schema object.
          ...
          // de-reference a json pointer
            ...
            resolution = merge(resolution, traverse(reference), true);
          } else {
            throw new Error(
  ...
  // if any other combination of arguments is provided we throw
    ...
    throw new TypeError(
      `expected first parameter to be object or array: ${schema}`
    );
  }
};
